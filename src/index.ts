...
export function incompleteRequestContinuation(
  ...
  let assistantToolCallIds: readonly string[] | undefined
  ...
  for (let index = turnStart + 1; index < endIndex; index += 1) {
    ...
    if (
      ...
      assistantToolCallIds = event.data.message.content.flatMap((block) => (
  ...
  if (step === undefined || !hasDurableUserInput) return undefined
  if (assistantToolCallIds === undefined) {
    return { turn, step, time, kind: 'incomplete-request' }
  }
  if (
    assistantToolCallIds.length > 0
